...
export function FulfillmentSelector({ value, onChange, supportsPickup, supportsDelivery, deliveryFee, freeDeliveryAbove, preparationTimeMinutes, subtotal }: FulfillmentSelectorProps) {
  const options = [
    supportsPickup ? { id: "pickup" as const, icon: Store, title: "Pickup", text: "Collect from Store", detail: `Ready in ${preparationTimeMinutes} minutes`, fee: "No delivery fee" } : null,
    supportsDelivery ? { id: "delivery" as const, icon: Truck, title: "Delivery", text: "Deliver to my address", detail: "Estimated within 1 hour after preparation", fee: freeDeliveryAbove > 0 && subtotal >= freeDeliveryAbove ? "Free delivery" : `Delivery fee ₹${deliveryFee.toLocaleString("en-IN")}` } : null,
