...
import { BadgeCheck, Crown, MapPin, ShieldCheck, Star } from "lucide-react";
...
function StoreBadgePills({ badges }: { badges?: Array<"verified" | "royal"> }) {
...
export function StoreCard({ store }: StoreCardProps) {
  ...
  return (
    <Link href={getStoreHref(store)} className="group block overflow-hidden rounded-[1.5rem] border border-stone-200 bg-white shadow-sm transition duration-300 hover:-translate-y-1 hover:shadow-lg dark:border-stone-800 dark:bg-zinc-900">
      ...
      <div className="p-5 pt-9"><div className="flex items-start justify-between gap-2"><div className="min-w-0 flex-1"><h3 className="text-base font-semibold text-stone-900 dark:text-stone-50">{displayName}</h3><StoreBadgePills badges={store.badges} /><div className="mt-1 flex items-center gap-2 text-sm text-stone-500 dark:text-stone-400"><Star className="h-3.5 w-3.5 fill-current text-amber-500" />{ratingLabel} <span>({store.totalReviews ?? 0} reviews)</span></div></div><span className={`rounded-full px-2 py-1 text-[10px] font-semibold ${open ? "bg-emerald-100 text-emerald-700" : "bg-stone-100 text-stone-500"}`}>{open ? "Open Now" : "Closed"}</span></div>
