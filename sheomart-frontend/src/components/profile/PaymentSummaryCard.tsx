  ...
  return (
    <section className="rounded-[2rem] border border-stone-200 bg-white p-6 shadow-sm dark:border-stone-800 dark:bg-stone-900">
      ...
      <div className="mt-5 space-y-3 rounded-[1.25rem] bg-stone-50 p-4 text-sm dark:bg-stone-950/60">
        ...
        <div className="flex justify-between gap-4"><span className="text-stone-500 dark:text-stone-400">Payment Status</span><span className="font-medium text-amber-700 dark:text-amber-300">{paymentStatus === "Paid" ? "Payment Received" : "Pending (Pay at Shop)"}</span></div>
      </div>
    </section>
  );
}
