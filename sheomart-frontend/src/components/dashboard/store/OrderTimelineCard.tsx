...
export function OrderTimelineCard({ order }: { order: StoreOrder }) {
  const currentStatus = (order as StoreOrder & { pickupStatus?: string }).pickupStatus ?? order.orderStatus ?? order.status ?? "ORDER_PLACED";
  const currentIndex = steps.findIndex((step) => step.status === currentStatus);
  const cancelled = currentStatus === "CANCELLED";
  const statusTimestampKeys = ["createdAt", "createdAt", "createdAt", "createdAt"];
    ...
    <DashboardCard title="Order timeline" description="Read-only status history for this order.">
      <div aria-label="Order status timeline" className="space-y-0">
        ...
        {steps.map((step, index) => {
          ...
          const timestamp = index === currentIndex
            ? getOrderValue(order, "statusUpdatedAt") ?? getOrderValue(order, statusTimestampKeys[index])
            : getOrderValue(order, statusTimestampKeys[index]);
          return (
