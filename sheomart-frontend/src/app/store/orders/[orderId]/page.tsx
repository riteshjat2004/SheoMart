...
const labels: Record<string, string> = { ORDER_PLACED: "Order Placed", PREPARING: "Preparing", READY_FOR_PICKUP: "Ready for Pickup", PICKED_UP: "Picked Up", CANCELLED: "Cancelled", DRAFT: "Draft", CONFIRMED: "Confirmed", PROCESSING: "Processing", PACKED: "Packed" };
...
const isPlusCustomer = (order: NonNullable<ReturnType<typeof useStoreOrder>["data"]>) => order.customerType?.toUpperCase() === "PLUS" || order.isPlusCustomer === true || order.customer?.isPlus === true;
...
export default function StoreOrderDetailsPage() {
  ...
  const paymentEligible = Boolean(order && isPlusCustomer(order) && (order.orderStatus ?? order.status) === "READY_FOR_PICKUP" && (order.paymentStatus ?? "").toUpperCase() === "PENDING");
  ...
  return <main className="mx-auto w-full max-w-7xl space-y-6 px-4 py-6 sm:px-6 lg:px-8">
    ...
    {order ? <>
      ...
      {paymentEligible ? <DashboardCard title="Pickup payment reminder" description="This PLUS customer can pay the full order amount at pickup."><div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between"><div><p className="text-sm text-stone-600 dark:text-stone-300">Collect {money(order.grandTotal)} in full before completing pickup.</p><p className="mt-1 text-xs text-stone-500">Partial payment is not supported.</p></div><Button type="button" onClick={() => setIsPaymentDialogOpen(true)} disabled={collectPaymentMutation.isPending}>{collectPaymentMutation.isPending ? "Collecting..." : "Collect Payment"}</Button></div></DashboardCard> : null}
      ...
      {isPaymentDialogOpen ? <div className="fixed inset-0 z-50 flex items-center justify-center bg-stone-950/60 p-4" role="dialog" aria-modal="true" aria-labelledby="collect-payment-title"><div className="w-full max-w-md rounded-xl bg-white p-6 shadow-xl dark:bg-stone-900"><h2 id="collect-payment-title" className="text-lg font-semibold text-stone-900 dark:text-stone-50">Collect full payment</h2><p className="mt-2 text-sm text-stone-600 dark:text-stone-300">Confirm collection of {money(order.grandTotal)}. This will mark the order as paid and picked up.</p><label className="mt-5 block text-sm font-medium text-stone-700 dark:text-stone-200">Payment method<select value={paymentMethod} onChange={(event) => setPaymentMethod(event.target.value as PickupPaymentMethod)} disabled={collectPaymentMutation.isPending} className="mt-2 h-10 w-full rounded-lg border border-stone-200 bg-white px-3 dark:border-stone-700 dark:bg-stone-950"><option value="CASH">Cash at Shop</option><option value="UPI">UPI at Shop</option><option value="CREDIT">Credit</option></select></label><div className="mt-6 flex justify-end gap-2"><Button type="button" variant="outline" onClick={() => setIsPaymentDialogOpen(false)} disabled={collectPaymentMutation.isPending}>Cancel</Button><Button type="button" onClick={() => collectPaymentMutation.mutate({ orderId, paymentMethod })} disabled={collectPaymentMutation.isPending}>{collectPaymentMutation.isPending ? "Collecting..." : "Confirm Payment"}</Button></div></div></div> : null}
    </> : null}
  </main>;
}