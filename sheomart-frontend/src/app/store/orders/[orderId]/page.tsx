  ...
  return <main className="mx-auto w-full max-w-7xl space-y-6 px-4 py-6 sm:px-6 lg:px-8">
    ...
    {order ? <>
      <section className="rounded-xl border border-stone-200 bg-white/80 p-5 shadow-sm dark:border-stone-800 dark:bg-stone-900/80">
        <div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between"><div><p className="text-xs font-semibold uppercase tracking-[0.16em] text-emerald-600">Seller order details</p><h1 className="mt-2 text-2xl font-semibold text-stone-900 dark:text-stone-50">Order {order.orderId}</h1><p className="mt-2 text-sm text-stone-500">{order.customerName ?? order.customer?.name ?? order.customer?.fullName ?? "Customer"}</p></div><div className="flex flex-wrap gap-2"><StatusBadge status={labels[getPickupStatus(order)] ?? getPickupStatus(order)} /><span className="inline-flex rounded-full bg-amber-100 px-2.5 py-1 text-xs font-semibold text-amber-700">Payment: {labels[order.paymentStatus ?? ""] ?? order.paymentStatus ?? "Unknown"}</span></div></div>
      ...
      <OrderActionsCard status={getPickupStatus(order) as "ORDER_PLACED" | "PREPARING" | "READY_FOR_PICKUP" | "PICKED_UP" | "CANCELLED"} isPending={updateStatusMutation.isPending} onStatusChange={(status) => updateStatusMutation.mutate({ orderId, status })} />
